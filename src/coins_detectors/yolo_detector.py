...
class YoloDetector(CoinsDetector):
    ...
    def detect(self, image_path, biggest_radius_coin_value=None):
        ...
        coins = self.get_coins_from_detections(detections_boxes, detections_classes)

        return coins

    def get_coins_from_detections(self, detections_boxes, detections_classes):
        coins = []
        for bounding_box, class_id in zip(detections_boxes, detections_classes):
            coins.append({
